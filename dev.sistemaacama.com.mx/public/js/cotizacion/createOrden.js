    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response.model, function (key, item) {
          ...
          tab += '<td><button class="btn btn-danger btn-sm" onclick="deletePuntoSol('+item.Id_punto+')" '+std+'><i class="fas fa-trash"></i></button> &nbsp; <button class="btn btn-info btn-sm" onclick="editPuntoMuestreo('+item.Id_punto+')" '+std+'><i class="fas fa-edit"></i></button></td>'
          tab += '</tr>'
        });
        sub.innerHTML = tab;
      }
