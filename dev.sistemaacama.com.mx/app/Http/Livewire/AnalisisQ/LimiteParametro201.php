...
use App\Models\Limite201;
use Illuminate\Support\Facades\DB;
use Livewire\Component;

class LimiteParametro201 extends Component
{
    public $idParametro;
    public $alert = false; 
    ...
    public function render()
    {
        ...
        $model = DB::table('limitepnorma_201')->where('Id_parametro',$this->idParametro)->get();
        if($model->count()) 
        {
            $model = DB::table('limitepnorma_201')->where('Id_parametro',$this->idParametro)->first();
        }else{
            $this->createLimites($this->idParametro);
            $model = DB::table('limitepnorma_201')->where('Id_parametro',$this->idParametro)->first();
        }

        return view('livewire.analisis-q.limite-parametro201',compact('model','parametro')); 
    }
    public function store()
    {
        $model = Limite201::find($this->idLimite);
