                ...
                response.model.forEach((item) => {
                    ...
                    let opciones = response.parametros
                        .map((param) => {
                            const isSelected = response.solParam[cont - 1]?.some(
                                (solP) => solP.Id_parametro === param.Id_parametro
                            )
                                ? "selected"
                                : "";
                    ...
                    tabMuestras += `
                        <tr>
                            ...
                            <td>
                                <textarea rows="2" cols="50" id="muestra${item.Id_muestra}">
                                    ${item.Muestra ?? ""}
                                </textarea>
                            </td>
                            <td>
                                <select class="select2Multiple form-control" id="parametros${item.Id_muestra}" multiple="multiple">
                                    ${opciones}
                                </select>
                            </td>
                            <td>
                                <select class="select2 form-control" id="normas${item.Id_muestra}">
                                    ${opciones2}
                                </select>
                            </td>
                            <td>
                                <button id="btnSaveMuestra${item.Id_muestra}" onclick="setSaveMuestra(${item.Id_muestra})" class="btn btn-success">
...
function getSubNormas(id){
    $.ajax({
        ...
        success: function(response){
            ...
            response.model.forEach(item => {
                const isSelected = solModel && parseInt(solModel.Id_subnorma) === parseInt(item.Id_subnorma);
                data += `<option value="${item.Id_subnorma}" ${isSelected ? 'selected' : ''}>(${item.Id_subnorma}) ${item.Clave}</option>`;
            });
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                if (response.data.length > 0) {
                    let contacto = response.data[0];
                    $('#idCont').val(contacto.Id_sucursal);
                    ...
                    // alert("No se encontraron contactos para esta sucursal.");
                }
            } else {
