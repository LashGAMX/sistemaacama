...
function setLiberarTodo() {
  $.ajax({
    type: "POST",
    url: base_url + "/admin/alimentos/setLiberarTodo",
    data: {
      idLote: idLote,
      _token: $('input[name="_token"]').val(),
    },
...
function setDetalleMuestra() {
  ...
  switch (parseInt(idArea)) {
    ...
    case 12:
    case 6:
      console.log("entro en este caso 3");
      $.ajax({
        type: "POST",
      ...
      break;
    default:
      console.log("entro en este caso default");
      $.ajax({
        type: "POST",
        url: base_url + "/admin/alimentos/setDetalleMuestra",
        data: {
...
function getDetalleMuestra(id) {
  ...
  $.ajax({
    type: "POST",
    url: base_url + "/admin/alimentos/getDetalleMuestra",
    ...
    success: function (response) {
      ...
      $.each(response.model, function (key, item) {
          ...
          default:
            if (item.Resultado != null) {
              ...
              tab +=
                ...
                '"><input id="resultadoCap' +
                item.Id_detalle +
                '" disabled style="width: 100px;" value="'+item.Resultado+'"></td>';
            }
        ...
        tab +=
          ...
          item.proceso.Periodo_analisis +
          '" id="pa' +
          item.Id_codigo +
          '">' +
          '&nbsp; <button type="button" onclick="setFechaPA(' +
          item.Id_codigo +
          ')" class="btn-success">' +
...
function setFechaPA(idCod) {
  const fecha = $("#pa" + idCod).val();
  ...
  $.ajax({
    type: "POST",
    url: base_url + "/admin/alimentos/setFechaPA2",
    data: {
      fecha: fecha,
      idCod: idCod,
  ...
  $.ajax({  
    ...
    success: function (response) {
      ...
      for (let i = 0; i < response.model.length; i++) {
        //console.log(response.folio[i]);
        estiloH = response.historial[i] == 1 ? "background-color:#e5e5ff;" : "";
        let fechaRecepcion = new Date(response.fecha[i]);
        let Hoy = new Date();
        let diasAnalisis = response.diasanalisis[i] ?? 0;
        ...
        if (parseInt(diasAnalisis) !== 0 && diasAnalisis !== null) {
          if (diasDiferencia < diasAnalisis) {
            color = "bg-success";
            mensaje = "Faltan Días(" + (diasAnalisis - diasDiferencia) + ")";
          } else if (diasDiferencia === diasAnalisis) {
            color = "bg-warning";
            mensaje = "Último día(" + diasDiferencia + ")";
        ...
        tab +=
          '<td style="' + estiloH + '">' + (response.norma[i] ?? "") + "</td>";
        tab +=
          '<td style="' + estiloH + '">' + (response.punto[i] ?? "") + "</td>";
...
function getLote() {
  if ($("#parametro").val() != "0") {
    ...
    $.ajax({
      type: "POST",
      url: base_url + "/admin/alimentos/getLote",
      data: {
        id: $("#parametro").val(),
        fecha: $("#fechaLote").val(),
        folio: $("#folio").val(),
...
function getPendientes() {
  ...
  $.ajax({
    type: "POST",
    url: base_url + "/admin/alimentos/getPendientes",
    data: {
      _token: $('input[name="_token"]').val(),
    },
    dataType: "json",
    ...
    success: function (response) {
      ...
      for (let i = 0; i < model.length; i++) {
        let fechaRecepcionObj = new Date(model[i][3]);
        let fechaRecepcion = new Date(model[i][3]);
        let hoy = new Date();
        let diasAnalisis = model[i][6];
        let diasDiferencia = Math.ceil(
          (hoy - fechaRecepcion) / (1000 * 60 * 60 * 24)
        );
        ...
        if (parseInt(diasAnalisis) !== 0 && diasAnalisis !== null) {
          if (diasDiferencia < diasAnalisis) {
            ...
            color = "bg-warning";
            mensaje = "Ultimo dia(" + diasDiferencia + ")";
          } else if (diasDiferencia > diasAnalisis) {
            color = "bg-danger";
            mensaje = "Fuera de Tiempo(" + diasDiferencia + ")";
          }
          ...
          color = "bg-info";
          mensaje = "No cuenta con limite de dias(" + diasAnalisis + ")";
        }

        tab += "<tr>";
        tab += "<td>" + model[i][0] + "</td>"; // Folio
        tab += "<td>" + model[i][1] + "</td>"; // Parametro
        tab += "<td>" + model[i][2] + "</td>"; // Matriz Alimentos
        tab += "<td>" + model[i][3] + "</td>"; // Fecha Recepción
        tab += "<td>" + model[i][4] + "</td>"; // Empresa
        tab += '<td class="' + color + '">' + mensaje + "</td>"; // Dias Analisis
        tab += "</tr>";
      }
