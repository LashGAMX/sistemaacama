$(document).ready(function () {
    ...
    $("#btnCrearSolicitud").dblclick(function () {
        alert("Se ha hecho doble clic en el botón.");
    });
...
function disabledInput() {
    switch (parseInt($("#idUser").val())) {
        case 1:
            ...
            $("#tipoMuestra").attr("disabled",false);
            $("#promedio").attr("disabled",false);
            $("#tipoReporte").attr("disabled",false);
            $("#tipoReporte2").attr("disabled",false);
...
function getDataUpdate() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            tabParam = true;
            $(
                "#intermediario  option[value=" + data.Id_intermediario + "]"
            ).attr("selected", true);
            $("#tipoServicio  option[value=" + data.Id_servicio + "]").attr(
                "selected",
                true
            );
            $("#tipoDescarga  option[value=" + data.Id_descarga + "]").attr(
                "selected",
                true
            );
            $("#frecuencia  option[value=" + data.Id_muestreo + "]").attr(
                "selected",
                true
            );
            $("#promedio  option[value=" + data.Id_promedio + "]").attr(
                "selected",
                true
            );
            $("#tipoMuestra  option[value=" + data.Id_muestra + "]").attr(
                "selected",
                true
            );
            $("#tipoReporte  option[value=" + data.Id_reporte + "]").attr(
                "selected",
                true
            );
            $("#tipoReporte2  option[value=" + data.Id_reporte2 + "]").attr(
                "selected",
                true
            );
...
function setContacto() {
    ...
    newModal(
        ...
        2,
        0,
        inputBtn("", "", "Guardar", "save", "success", "createContacto()")
    );
}
        ...
        success: function (response) {
            ...
            newModal(
                ...
                inputBtn(
                    ...
                    "success",
                    "storeContacto(" +
                        response.model.Id_contacto +
                        "," +
                        response.model.Id_cliente +
                        ")"
                )
            );
        },
...
function getPuntoMuestro() {
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log(response);
            if (response.sw == true) {
                //validacion cuadno se crea una solicitud y se intenta agregar más puntos de muestreo.
                alert(
                    "No puedes agregar más puntos de muestreo una vez generada la Solicitud"
                );
            } else {
                $.each(response.model, function (key, item) {
            ...
            console.log(element)
            itemModal[1] = element;
            newModal( "divModal", "setPuntoMuestro", "Agregar punto muestreo", "lg", 1, 1, 1,
                inputBtn( "btnAddPunto", "btnAddPunto", "Guardar", "save", "success","btnAddPunto()")
            );
        },
    });
}
...
function updateParametroSol() {
    ...
    $.ajax({
        ...
        async: false,
        success: function (response) {
            (parametros = response.model), alert("Parametros actualizados");
            createTabParametros();
        },
    });
}
...
//         if (
//             confirm(
//                 "Nota: Una vez creado la orden ya no se podra editar, Deseas continuar?"
//             )
//         ) {
//             $.ajax({
...
function setCreateOrden() {
    if ($("#folio").val() != "") {
        if (confirm("Nota: Una vez creada la orden ya no se podrá editar. ¿Deseas continuar?")) {
            $.ajax({
                url: base_url + "/admin/cotizacion/solicitud/setCreateOrden", //archivo que recibe la peticion
                type: "POST", 
                data: {
