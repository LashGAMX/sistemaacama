...
CREATE VIEW ViewPuntoMuestreoSolSir as SELECT pu.Id_punto as Id_puntoSol,pu.Id_solicitud,pu.Id_muestreo,sir.Id_punto,sir.Id_sucursal,sir.Titulo_consecion,sir.Punto,sir.Anexo,sir.Siralab,sir.Pozos,sir.Cuerpo_receptor,sir.Uso_agua 
...
CREATE VIEW ViewLoteDetalleEspectro as SELECT det.*,sol.Folio_servicio,param.Clave_norma,param.Parametro,control.Control,cod.Codigo,cod.Num_muestra
...
CREATE VIEW ViewCodigoParametro AS SELECT cod.*,sol.Folio_servicio,sub.Id_subnorma,sub.Id_norma,sub.Norma,sub.Clave,pa.Parametro, pa.Id_simbologia, pa.Simbologia, pa.Id_tipo_formula,pa.Tipo_formula, pa.Unidad, pa.Metodo_prueba, pa.Clave_metodo FROM codigo_parametro as cod
