                 ...
                 <div class="col-md-6">
                    ...
                    <form action="https://sigplusweb.com/sign_chrome_ff_sigplusextlite.html#" name="FORM1" style="display: flex">
                        <p>
                            ...
                            <button id="btnFirma" name="btnFirma" type="button">Guardar<i class="fas fa-save"></i></button>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div style="width: 100%; overflow-x: auto;">
                                <table id="tableServicios" class="table table-bordered" style="width: 100%; font-size: 10px; table-layout: fixed;">
                                    <thead>
                                          <tr>
                                               <!-- Columna Id muy pequeña -->
                                               <th style="width: 5%;">
                                                   <input type="text" placeholder="Buscar Id" style="width: 100%;">
                                               </th>
                                               <!-- Columnas principales un poco más anchas -->
                                               <th style="width: 10%;">
                                                   <input type="text" placeholder="Buscar Folio" style="width: 100%;">
                                               </th>
                                               <th style="width: 15%;">
                                                   <input type="text" placeholder="Buscar Cliente" style="width: 100%;">
                                               </th>
                                               <th style="width: 15%;">
                                                   <input type="text" placeholder="Buscar Norma" style="width: 100%;">
                                               </th>
                                               <th style="width: 15%;">
                                                   <input type="text" placeholder="Buscar Servicio" style="width: 100%;">
                                               </th>
                                           </tr>
                                    </thead>
                                    <tbody></tbody>
...
<script type="text/javascript">
	...
	try{
		...
		script.onerror = function () {
			//console.log("Unable to load required SigPlusExtLite extension wrapper. Confirm extension is up-to-date, installed, and enabled, the NMH/SDK is installed, and SigPlus OCX is installed.");
		}
		script.src = url;
		...
		try{
			...
			if(extVersion != null){
				...
				extP.innerHTML += "<b>Extension Version: </b>"+extVersion;
				 ...
				 //console.log("Unable to get the extension version. Confirm it is installed and enabled.");
			}
			...
			if(nmhVersion != null){
				...
				if(sigplusVersion != null){
					...
					if(lastError != null){
						//console.log("Issue getting SigPlus OCX version information. "+lastError);
					} else {
						//console.log("Unable to get SigPlus OCX version information");
					}
				}
				...
				let lastError = await global.GetLastError();
				if(lastError != null){
					//console.log("Issue getting NMH/SDK version information. "+lastError);
				} else {
					//console.log("Unable to get NMH/SDK version information");
				}
			}
			
			
		} catch(ex){
			//console.log("Unable to get version information. Confirm extension is installed and enabled, the NMH/SDK is installed, and SigPlus OCX is installed.");
		}
	}
	...
	async function StartSign()
	 {   
	    ...
	    if (!isInstalled) {
	        //console.log("SigPlusExtLite extension is either not installed or disabled. Please install or enable extension.");
			return;
	    }	
		...
		try{
			...
			if(lastError != null && lastError != ""){
				if(lastError == "The signature does not have enough points to be valid."
					...
					//console.log("SigPlusExtLite Error: On Signing - " + lastError);
				}
			} else {	
...
<script src="{{asset('/public/js/informes/informes2.js')}}?v=1.0.8"></script>
@stop