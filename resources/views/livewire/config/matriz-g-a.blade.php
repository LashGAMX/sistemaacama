<div> 
                ...
                <tbody>
                    @foreach ($model as $item)
                        ...
                        @else
                            <tr>
                        @endif
                            <td>{{$item->Id_matraz}}</td>
                            <td>{{$item->Num_serie}}</td>
                            <td>{{$item->Peso}}</td> 
                            <td>{{$item->Min}}</td>
                            <td>{{$item->Max}}</td>
                            <td><button class="btn btn-warning"
                                wire:click="setData('{{$item->Id_matraz}}','{{$item->Num_serie}}','{{$item->Peso}}','{{$item->Min}}','{{$item->Max}}')"
                                data-toggle="modal" data-target="#modalMatraz"> <i class="voyager-edit"></i> Editar</button></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div wire:ignore.self class="modal fade" id="modalMatraz" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
