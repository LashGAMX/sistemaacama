    ...
    <div class="row">
      ...
      <div class="col-md-4">
        ...
        <label for="">Matriz</label>
          <input type="text" wire:model='matriz' class="form-control" placeholder="Matriz">
  ...
  @endif
    <table class="table table-hover table-striped">
        ...
        <tr>  
          ...
          <td>{{$item->Id_matriz_parametro}}</td>
          <td>{{$item->Matriz}}</td>          
          <td>{{$item->created_at}}</td>
          <td>{{$item->updated_at}}</td>
          <td>
            <button type="button" class="btn btn-primary" wire:click="setData('{{$item->Id_matriz_parametro}}','{{$item->Matriz}}')"  data-toggle="modal" data-target="#modalMatriz"><i class="voyager-edit"></i> <span hidden-sm hidden-xs>editar</span> </button>
          </td>
          {{-- </form>  --}}
        </tr>
    @endforeach
    ...
    <div wire:ignore.self class="modal fade" id="modalMatriz" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form wire:submit.prevent="store">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Modificar matriz</h5>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col-md-12">
                    <input type="text" wire:model="idMatriz" hidden>
                    <label for="">Matriz</label>
                    <input type="text" wire:model='matriz' class="form-control" placeholder="Matriz">
                    @error('matriz') <span class="text-danger">{{ $message  }}</span> @enderror
                </div>
